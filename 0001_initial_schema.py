...
def upgrade() -> None:
    ...
    # ── Enum types (created once, reused by columns) ──────────────────────
    currency_enum            = sa.Enum("KSH", "UGX",                                                     name="currency")
    userrole_enum            = sa.Enum("admin", "manager", "cashier", "waiter",                          name="userrole")
    orderstatus_enum         = sa.Enum("placed", "served", "cleared",                                     name="orderstatus")
    transactionstatus_enum   = sa.Enum("pending", "completed", "voided", "refunded",                     name="transactionstatus")
    paymentmethod_enum       = sa.Enum("cash", "card", "mobile_money", "credit",                         name="paymentmethod")
    purchaseorderstatus_enum = sa.Enum("draft", "sent", "partially_received", "received", "cancelled",   name="purchaseorderstatus")
    movementtype_enum        = sa.Enum("purchase", "sale", "adjustment", "return_in", "return_out", "damage", name="movementtype")

    for e in [currency_enum, userrole_enum, orderstatus_enum, transactionstatus_enum,
              paymentmethod_enum, purchaseorderstatus_enum, movementtype_enum]:
    ...
    # ── products (dual-currency pricing) ─────────────────────────────────
    op.create_table(
        "products",
        sa.Column("id",              sa.Integer(),               autoincrement=True, nullable=False),
    ...
    # ── orders (lifecycle: placed → served → cleared) ─────────────────────
    op.create_table(
        "orders",
        sa.Column("id",            sa.Integer(),               autoincrement=True, nullable=False),
